...
public class MancalaDataStructure implements Serializable {
    ...
    private List<Countable> data = new ArrayList<>();  //array list with 14 slots, all countable to data structure
    private int iteratorPos = 0;
    private int playerSkip = PLAYER_TWO; //needs to know what player store to skip but doesnt care who player is
    ...
    public int addStones(final int pitNum, final int numToAdd) {
        final Countable pit = data.get(pitPos(pitNum));  //converts number to position in array list
     ...
     */
    public int addToStore(final int playerNum, final int numToAdd) {   //player 1 pits 1-6, player 2 pits 7-12
     ...
     */
    public int getStoreCount(final int playerNum) {  //you dont get the actual pits, just the numbers
        final Countable store = data.get(storePos(playerNum));
        return store.getStoneCount();
    }
    ...
    public void setIterator(final int startPos, final int playerNum, final boolean skipStartPit) {   
        iteratorPos = pitPos(startPos); //sets postion
        setSkipPlayer(playerNum);
        if (skipStartPit) {
            setSkipPit(startPos);
        }
    }
    // to know where to start the next time around, tell it where to start
